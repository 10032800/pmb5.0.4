...
// +-------------------------------------------------+
// $Id: search.class.php,v 1.291.2.13 2018-02-01 16:40:33 plmrozowski Exp $
...
class search {
	...
	protected function parse_search_file() {
		...
		if(!$this->full_path){
			...
			if (file_exists($include_path."/search_queries/".$this->fichier_xml."_subst.xml")) {
			...
			if (file_exists($this->full_path.$this->fichier_xml."_subst.xml")) {
	...
	public function make_search($prefixe="") {
		...
		for ($i=0; $i<count($search); $i++) {
			...
			if ($s[0]=="f") {
				...
				for ($j=0; $j<count($field); $j++) {
					...
					if(file_exists($include_path."/search_queries/dynamics/dynamic_search_".$this->pp[$s[0]]->t_fields[$s[1]]['TYPE'].".class.php")) {
						...
						$main = $dynamic_class->get_query($field[$j]);
					} else {
						...
						if ($q["MULTIPLE_WORDS"]) {
							...
							$field[$j]=str_replace('*', '%', $field[$j]);
							$main=str_replace("!!p!!",addslashes($field[$j]),$main);
							$main=str_replace("!!p1!!",addslashes($field1[$j]),$main);								
						}
			...
			switch (${$inter}) {
				case "and":
					//TODO AR/AP Test, first criteria;
			...
			if (array_key_exists($s[0],$this->pp)) {
				...
				for ($j=0; $j<count($field); $j++) {
					...
					if(file_exists($include_path."/search_queries/dynamics/dynamic_search_".$this->pp[$s[0]]->t_fields[$s[1]]['TYPE'].".class.php")) {
						...
						$dynamic_class_name = "dynamic_search_".$this->pp[$s[0]]->t_fields[$s[1]]['TYPE'];
						$dynamic_class = new $dynamic_class_name($s[1],$s[0], $i,$df,$this);
						$field_aff[$j] = $dynamic_class->make_human_query($field[$j]);
					} else {
...
// 			}
	...
	public function show_form($url,$result_url,$result_target='',$memo_url='',$only_selected_fields='false') {
		...
		if(!$this->groups_used){
			...
			foreach($this->groups as $group_id => $group){
				if(isset($fields_array[$group_id])){ //On a des champs définis pour le groupe courant
	...
	public function show_dnd_form() {
		...
		return str_replace('!!search_controller_class!!', $search_controller_class, $extended_search_dnd_tpl);
	}
	...
	public function reduct_search() {
		...
		for ($i=0; $i<count($search); $i++) {
			...
			if (((string)$field[0]!="" || (string)$field1[0]!="") || ($this->op_empty[${$op}])) {
				$tt[$it]=$i;
				$it++;
			}
		}
	...
	protected function gen_temporary_table($table_name, $main='', $with_pert=false) {
		$requete="create temporary table ".$table_name." ENGINE=".$this->current_engine." ".$main;
		@pmb_mysql_query($requete);
		if (!pmb_mysql_num_rows(pmb_mysql_query("show columns from ".$table_name." like 'idiot'"))) {		
			$requete="alter table ".$table_name." add idiot int(1)";
			@pmb_mysql_query($requete);
		}
		$requete="alter table ".$table_name." add unique(".$this->keyName.")";
		@pmb_mysql_query($requete);
		if($with_pert) {
			if (!pmb_mysql_num_rows(pmb_mysql_query("show columns from ".$table_name." like 'pert'"))) {
				$requete="alter table ".$table_name." add pert decimal(16,1) default 1";
				@pmb_mysql_query($requete);
			}
		}
	}
    ...
    protected function show_objects_results($table, $has_sort) {
		...
		while ($r=pmb_mysql_fetch_object($result)) {
			...
			print "<div class='row'>".$nt->result."</div>";
		}
    ...
    protected function get_display_actions() {
		...
		if ($dsi_active && !$opac_view_id) {
			...
			$display_actions .= "<span class='space-disable'>&nbsp;</span><input  type='button' class='bouton' onClick=\"document.forms['transform_dsi'].submit(); \" value=\"".$mess_bouton."\"/>
						<form name='transform_dsi' style='display:none;' method='post' action='./dsi.php'>";
			if ($priv_pro=="PRI") {
				$display_actions .= "
				<input type=hidden name='categ' value='bannettes' />
		...
		if ($pmb_opac_view_activate) {
			if($opac_view_id){
				$mess_bouton = $msg['opac_view_sauvegarder_equation'] ;
				$display_actions .= "
					<span class='space-disable'>&nbsp;</span><input  type='button' class='bouton' onClick=\"document.forms['transform_opac_view'].submit(); \" value=\"".$mess_bouton."\"/>
			...
			if ($current!==false) {
				...
				$display_icons .= "<span class='space-disable'>&nbsp;</span>
									<a href='#' onClick=\"openPopUp('./print_cart.php?current_print=$current&action=print_prepare$tri_id_info','print',600,700,-2,-2,'scrollbars=yes,menubar=0,resizable=yes'); return false;\">
										<img src='./images/basket_small_20x20.gif' border='0' align='center' alt=\"".$msg["histo_add_to_cart"]."\" title=\"".$msg["histo_add_to_cart"]."\">
									</a>
									...
									<a href='#' onClick=\"openPopUp('./print.php?current_print=$current&action_print=print_prepare$tri_id_info','print',500,600,-2,-2,'scrollbars=yes,menubar=0'); w.focus(); return false;\">
										<img src='./images/print.gif' border='0' align='center' alt=\"".$msg["histo_print"]."\" title=\"".$msg["histo_print"]."\"/>
									</a>";
				$display_icons .= "<span class='space-disable'>&nbsp;</span>
									<a href='#' onClick=\"openPopUp('./download.php?current_download=$current&action_download=download_prepare".$tri_id_info."','download',500,600,-2,-2,'scrollbars=yes,menubar=0'); return false;\">
										<img src='./images/upload.gif' border='0' align='center' alt=\"".$msg["docnum_download"]."\" title=\"".$msg["docnum_download"]."\"/>
									</a>";
				if ($pmb_allow_external_search){
					...
					$display_icons .= "<span class='space-disable'>&nbsp;</span>
										<a ".$tag_a."  title='".$msg["connecteurs_external_search_sources"]."'>
											<img src='./images/external_search.png' border='0' align='center' alt=\"".$msg["connecteurs_external_search_sources"]."\"/>
										</a>";
				}
				if ($nb_results<=$pmb_nb_max_tri) {
